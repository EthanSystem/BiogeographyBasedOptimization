...
for GenIndex = 1 : OPTIONS.Maxgen   % 从第一代开始
    ...
    for i=1:OPTIONS.popsize
        ExportData.Details(SET.indexOfOptimizationFunction,SET.indexOfBenchmarkFunction).EachPopulationInEachGenerationAndMonte(GenIndex+1,SET.monte_index).Population(i).chrom=Population(i).chrom;
        ExportData.Details(SET.indexOfOptimizationFunction,SET.indexOfBenchmarkFunction).EachPopulationInEachGenerationAndMonte(GenIndex+1,SET.monte_index).Population(i).FitnessCost=Population(i).FitnessCost;
        ExportData.Details(SET.indexOfOptimizationFunction,SET.indexOfBenchmarkFunction).EachPopulationInEachGenerationAndMonte(GenIndex+1,SET.monte_index).Population(i).Cost=Population(i).cost;
    end
    ExportData.Details(SET.indexOfOptimizationFunction,SET.indexOfBenchmarkFunction).BestAndMeanInEachGenerationAndMonte(GenIndex+1).generation(SET.monte_index)=GenIndex;
    ExportData.Details(SET.indexOfOptimizationFunction,SET.indexOfBenchmarkFunction).BestAndMeanInEachGenerationAndMonte(GenIndex+1).best(SET.monte_index)=MinCost(end);
    ExportData.Details(SET.indexOfOptimizationFunction,SET.indexOfBenchmarkFunction).BestAndMeanInEachGenerationAndMonte(GenIndex+1).mean(SET.monte_index)=AvgCost(end);
...
ExportData.Details(SET.indexOfOptimizationFunction,SET.indexOfBenchmarkFunction).DiversityMeasure(SET.monte_index)=Hamming;
