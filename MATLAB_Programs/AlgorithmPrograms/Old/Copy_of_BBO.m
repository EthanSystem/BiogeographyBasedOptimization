...
for GenIndex = 1 : OPTIONS.Maxgen   % 从第一代开始
    ...
    ExportData.Details(SET.indexOfOptimizationFunction,SET.indexOfBenchmarkFunction).BestAndMeanOfGeneration(GenIndex+1).generation(SET.monte_index)=GenIndex;
    ExportData.Details(SET.indexOfOptimizationFunction,SET.indexOfBenchmarkFunction).BestAndMeanOfGeneration(GenIndex+1).best(SET.monte_index)=MinCost(end);
    ExportData.Details(SET.indexOfOptimizationFunction,SET.indexOfBenchmarkFunction).BestAndMeanOfGeneration(GenIndex+1).mean(SET.monte_index)=AvgCost(end);
